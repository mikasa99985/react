...
const Payment = () => {
  ...
  // console.log({ id: query.get('id'), name: query.get('name'), img: localStorage.getItem('img'), price:query.get('price') });
  ...
  function buy() {
    if (isLogin) {
      database.ref(`users/${auth.currentUser.uid}/library/${query.get('id')}`).set({
        name: query.get('name'),
        img: localStorage.getItem('img')
      }).then(() => {
        database.ref(`users/${auth.currentUser.uid}/my_cards/${query.get('id')}`).remove().then(function () {
  ...
  return (
    <>
      <div className="d-flex flex-column flex-lg-row justify-content-evenly align-items-center p-box">
        <div className="">
          <img src={localStorage.getItem('img')} className='p-g-img' alt="" />
          <h2 className='my-3'>{query.get('name').length >= 25 ? query.get('name').slice(0, 25) + '...' : query.get('name')}</h2>
          <h3 className='my-3'>{query.get('price') == 0 ? 'FREE' : '₹' + query.get('price')}</h3>
        </div>

        <div className='d-flex flex-column justify-content-evenly align-items-center p-w-100'>
...

          {
            query.get('price') != 0 ?
                  ...
                  {/* Successful */}
                  {
                    state == 'successful' ?
                      <div className="alert alert-success h-100 d-flex justify-content-center align-items-center" role="alert">
                        <h2>₹ {query.get('price')} Successful Payment !!!</h2>
                      </div>
                      :
                      <></>
                  }
              ...
              :
              <div className="d-flex flex-column justify-content-evenly align-items-center payment-process-box">
                {
                  state =='select'?
                  <>
                    <div className="">
                      ...
                      <button type="submit" className="btn-cus btn-blue-glow w-75" onClick={GetNow} style={{ borderRadius: '8px', padding: '8px' }}>Get Now</button>
                  </>
                  :
                  <></>
                }
